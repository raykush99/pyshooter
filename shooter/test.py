...
speed_square = {
    "name": "speed_square",
    "position": [250, 50],
    "sprite": pygame.image.load("sprites/speed_square.png").convert_alpha(),
    "rect": [0, 0, 0, 0]
}
fire_square = {
    "name": "fire_square",
    "position": [350, 50],
    "sprite": pygame.image.load("sprites/fire_square.png").convert_alpha(),
    "rect": [0, 0, 0, 0]
}
sheild_square = {
    "name": "sheild_square",
    "position": [450, 50],
    "sprite": pygame.image.load("sprites/sheild_square.png").convert_alpha(),
    "rect": [0, 0, 0, 0]
}
...
# Speed boost control
    ...
    if pygame.time.get_ticks() >= timer_speed:
        ...
        speed_active = False
    if speed_active == True:
        pygame.draw.rect(screen, health_color, (speed_square["position"][0], speed_square["position"][1] + 40, (timer_speed - pygame.time.get_ticks()) / 100, 5))
#Rapid fire
    if keys[pygame.K_1]:
    ...
    if pygame.time.get_ticks() <= timer_fire:
        ...
        fire_active = False
    if fire_active == True:
        pygame.draw.rect(screen, health_color, (fire_square["position"][0], fire_square["position"][1] + 40, (timer_fire - pygame.time.get_ticks()) / 100, 5))
#sheild
    if keys[pygame.K_2]:
        sheild_active = True
        player["sprite"] = pygame.image.load("sprites/player_sheild.png").convert_alpha()
        timer_sheild = pygame.time.get_ticks() + 5000  
    if timer_sheild <= pygame.time.get_ticks():
        player["sprite"] = pygame.image.load("sprites/player.png").convert_alpha()
    else:
        sheild_active = True
    if sheild_active == True:
        pygame.draw.rect(screen, health_color, (sheild_square["position"][0], sheild_square["position"][1] + 40, (timer_sheild - pygame.time.get_ticks()) / 100, 5))
    ...
    for enemy_clone in enemies[:]:
        ...
        rect_enemy = pygame.Rect(enemy_clone["position"][0], enemy_clone["position"][1], enemy["radius"]*2, enemy["radius"]*2)
        if rect_enemy.colliderect(sprite_rect) and sheild_active == False:
                    print("Got attacked!")
                    health -= 1
                    try:
                        enemies.remove(enemy_clone)
    ...
    screen.blit(speed_square["sprite"], speed_square["position"])
    screen.blit(fire_square["sprite"], fire_square["position"])
    screen.blit(sheild_square["sprite"], sheild_square["position"])
    if health < 10:
